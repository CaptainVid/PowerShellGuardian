...
static const wchar_t* APP_NAME = L"PowerShell Guardian Control Center 1.1.1";
...
enum ControlId {
  ...
  ID_TOGGLE_UNATTENDED, ID_SECURITY, ID_LOGS, ID_EMERGENCY,
...
struct Session {
  ...
  bool approved=false, denied=false, tokenDelivered=false;
  bool unattended=false, suspiciousTerminated=false, windowContinuesBurst=false;
...
static const int FILE_BURST_RESTART_GRACE_SECONDS=60;
...
static std::string JsonGet(const std::string& j,const std::string& key) {
  return powershellguardian_json::ScalarField(j,key);
}
...
static std::string Base64(const BYTE* data,DWORD size){
  ...
  if(!encoded.empty()&&encoded.back()==0)encoded.pop_back();return encoded;
}
...
static void Audit(const std::string& session,const std::string& command,const std::string& decision,const std::string& result) {
  std::wstring path=g_base+L"\\logs\\audit.jsonl"; std::ofstream f(path.c_str(),std::ios::binary|std::ios::app);
...
static std::string DeviceHash() {
  HKEY k; wchar_t value[256]={}; DWORD size=sizeof(value),type=0; std::wstring id;
  if(RegOpenKeyExW(HKEY_LOCAL_MACHINE,L"SOFTWARE\\Microsoft\\Cryptography",0,KEY_READ|KEY_WOW64_64KEY,&k)==ERROR_SUCCESS){
    if(RegQueryValueExW(k,L"MachineGuid",nullptr,&type,(BYTE*)value,&size)==ERROR_SUCCESS)id=value; RegCloseKey(k);
  }
  wchar_t name[256]={}; DWORD n=256; GetComputerNameW(name,&n); id+=L"|"; id+=name; return Sha256(WideToUtf8(id));
}
...
static void SaveSessions() {
  ...
  for(auto& s:g_sessionData) if(s.approved&&s.expires>Now()){
    ...
    f<<s.id<<'|'<<s.created<<'|'<<s.lastActive<<'|'<<s.expires<<'|'<<Protect(s.token+"|"+s.deviceHash)
      ...
      <<'|'<<(s.windowContinuesBurst?1:0)<<'|'<<Protect(files.str())<<"\n";
  }
}
...
static void LoadSessions() {
  ...
  while(std::getline(f,line)){std::vector<std::string>x;size_t p=0,q;
    ...
    if(x.size()!=5&&x.size()!=13)continue; std::string secret=Unprotect(x[4]);size_t z=secret.rfind('|');if(z==std::string::npos)continue;
    ...
    s.tokenHash=Sha256(s.token+"|"+s.deviceHash);s.approved=true;s.tokenDelivered=true;
    if(x.size()==13){
      s.unattended=x[5]=="1";s.denied=x[6]=="1";s.suspiciousTerminated=x[7]=="1";
      s.fileWindowStart=_strtoi64(x[8].c_str(),nullptr,10);s.fileWindowCount=atoi(x[9].c_str());s.fullWindowStreak=atoi(x[10].c_str());s.windowContinuesBurst=x[11]=="1";
...
static bool ConstantEqual(const std::string&a,const std::string&b){
  if(a.size()!=b.size())return false; unsigned char d=0;for(size_t i=0;i<a.size();++i)d|=(unsigned char)(a[i]^b[i]);return d==0;
}

static Session* FindSessionToken(const std::string& token) {
  std::string dev=DeviceHash(), hash=Sha256(token+"|"+dev); long long now=Now();
  for(auto& s:g_sessionData) if(s.approved&&!s.denied&&s.expires>now&&s.deviceHash==dev&&ConstantEqual(s.tokenHash,hash))return &s;
  return nullptr;
}
...
static void LoadWhitelist(){
  ...
  if(powershellguardian_json::StringArrayField(json,"allowed",configured)){
  ...
  if(JsonGet(json,"default")!="approval_required"){
    ...
    migrated<<"],\n  \"approval_required\": [\"execute_command\", \"powershell\", \"file_write\", \"install\", \"delete\", \"registry\", \"network_change\"],\n  \"default\": \"approval_required\"\n}\n";
...
static void RotateFileWindow(Session& s,long long now){
  ...
  if(s.fileWindowStart<=0){s.fileWindowStart=now;s.fileWindowCount=0;s.windowContinuesBurst=false;return;}
  if(now<s.fileWindowStart+seconds)return;
  bool immediate=s.fileWindowCount>=g_fileLimit&&now<=s.fileWindowStart+seconds+FILE_BURST_RESTART_GRACE_SECONDS;
  if(!immediate){s.fullWindowStreak=0;s.burstFiles.clear();}
  s.fileWindowStart=now;s.fileWindowCount=0;s.windowContinuesBurst=immediate;
}
...
static bool RecordCreatedFiles(Session& s,const std::vector<std::string>& files,long long now){
  if(files.empty())return false;RotateFileWindow(s,now);s.fileWindowCount+=(int)files.size();s.burstFiles.insert(s.burstFiles.end(),files.begin(),files.end());
  Audit(s.id,"FILE CREATION","SESSION AUTO",FileAuditSummary(s));
  if(s.fileWindowCount<g_fileLimit)return false;
  s.fullWindowStreak=s.windowContinuesBurst?s.fullWindowStreak+1:1;s.windowContinuesBurst=false;
  if(!g_suspiciousGuardEnabled||s.fullWindowStreak<g_suspiciousFullWindows)return false;
  s.unattended=false;s.denied=true;s.suspiciousTerminated=true;
  for(auto& command:g_commandData)if(command.sessionId==s.id&&command.status=="WAITING APPROVAL"){command.status="REJECTED";command.result="Session terminated due to suspicious activity";}
  Audit(s.id,"SESSION TERMINATED","AUTOMATIC SAFETY",FileAuditSummary(s));return true;
}
...
static std::string RunPowerShell(const std::string& script) {
  ...
  if(!CreatePipe(&rd,&wr,&sa,0))return "ERROR: CreatePipe"; SetHandleInformation(rd,HANDLE_FLAG_INHERIT,0);
  ...
  STARTUPINFOW si{sizeof(si)};PROCESS_INFORMATION pi{};si.dwFlags=STARTF_USESTDHANDLES|STARTF_USESHOWWINDOW;si.wShowWindow=SW_HIDE;
  ...
  std::string output;char b[4096];DWORD got=0;while(ReadFile(rd,b,sizeof(b),&got,nullptr)&&got){if(output.size()<200000)output.append(b,got);}
  WaitForSingleObject(pi.hProcess,900000);DWORD ec=0;GetExitCodeProcess(pi.hProcess,&ec);CloseHandle(pi.hThread);CloseHandle(pi.hProcess);CloseHandle(rd);
  if(output.size()>200000)output.resize(200000);return "EXIT="+std::to_string(ec)+"\n"+output;
}

static std::string SystemInfo() {
  OSVERSIONINFOW os{sizeof(os)};
  GetVersionExW(&os);
  MEMORYSTATUSEX mem{sizeof(mem)};GlobalMemoryStatusEx(&mem);SYSTEM_INFO si;GetNativeSystemInfo(&si);
...
static std::string StatusJson() {
  size_t active=0,pending=0;long long now=Now();for(auto&s:g_sessionData)if(s.approved&&!s.denied&&s.expires>now)active++;
...
static std::string HandleGateway(const std::string& req) {
  ...
  if(action=="new_session"){
    ...
    Session s;s.code=std::to_string(code);s.id="MOM-"+s.code+"-"+NewId("").substr(0,6);s.created=s.lastActive=Now();s.expires=s.created+g_timeoutMinutes*60;
    g_sessionData.push_back(s);Audit(s.id,"NEW SESSION REQUEST","WAITING","CODE "+s.code);
  ...
  if(action=="session_status"){
    std::string id=JsonGet(req,"session_id");for(auto&s:g_sessionData)if(s.id==id){
      ...
      if(!s.approved)return "{\"ok\":true,\"status\":\"WAITING APPROVAL\"}";
      if(s.expires<=Now())return "{\"ok\":true,\"status\":\"EXPIRED\"}";
      if(!s.tokenDelivered){s.tokenDelivered=true;return "{\"ok\":true,\"status\":\"APPROVED\",\"session_token\":\""+JsonEscape(s.token)+"\",\"expires_at\":"+std::to_string(s.expires)+"}";}
      return "{\"ok\":true,\"status\":\"APPROVED\",\"token_already_delivered\":true,\"expires_at\":"+std::to_string(s.expires)+"}";
    }return "{\"ok\":false,\"error\":\"session not found\"}";
  }
  if(action=="command"){
    if(g_emergency)return "{\"ok\":false,\"error\":\"emergency lock active\"}";
    std::string token=JsonGet(req,"session_token"),name=JsonGet(req,"command"),payload=JsonGet(req,"payload");Session*s=FindSessionToken(token);
    if(!s)return "{\"ok\":false,\"error\":\"invalid or expired session token\"}";
    if(!ValidCommandName(name))return "{\"ok\":false,\"error\":\"invalid command name\"}";
    s->lastActive=Now();s->expires=s->lastActive+g_timeoutMinutes*60;SaveSessions();std::string risk=RiskFor(name);bool deleteIntent=ContainsDeleteIntent(name,payload);
    if(deleteIntent){
    ...
    if(risk=="LOW"||s->unattended){
      PendingCommand automatic;automatic.sessionId=s->id;automatic.name=name;automatic.payload=payload;automatic.risk=risk;
      bool ambiguousBulk=false;std::vector<std::string> createdFiles=DetectCreatedFiles(name,payload,ambiguousBulk);int retryAfter=0;std::string blocked;
      if(ambiguousBulk){blocked="BLOCKED: bulk or unbounded file creation is not allowed in an unlocked session";Audit(s->id,name,"FILE SAFETY",blocked);return "{\"ok\":false,\"status\":\"BLOCKED\",\"result\":\""+JsonEscape(blocked)+"\"}";}
      if(!CanCreateFiles(*s,(int)createdFiles.size(),Now(),blocked,retryAfter)){Audit(s->id,name,"FILE RATE LIMIT",blocked);SaveSessions();return "{\"ok\":false,\"status\":\"BLOCKED\",\"retry_after_seconds\":"+std::to_string(retryAfter)+",\"result\":\""+JsonEscape(blocked)+"\"}";}
      std::string result=ExecuteApproved(automatic);bool failed=result.rfind("ERROR",0)==0||result.rfind("BLOCKED",0)==0;Audit(s->id,name,"AUTO",failed?"FAILED":"SUCCESS");
      bool terminated=!failed&&RecordCreatedFiles(*s,createdFiles,Now());SaveSessions();if(g_main)PostMessageW(g_main,WM_MS_REFRESH,0,0);
      if(terminated)return "{\"ok\":true,\"status\":\"SESSION_TERMINATED_SUSPICIOUS_ACTIVITY\",\"result\":\""+JsonEscape(result)+"\"}";
      return "{\"ok\":"+std::string(failed?"false":"true")+",\"status\":\""+(failed?"FAILED":"SUCCESS")+"\",\"risk\":\""+JsonEscape(risk)+"\",\"approval\":\""+(s->unattended?"SESSION_AUTO":"WHITELIST")+"\",\"result\":\""+JsonEscape(result)+"\"}";
    }
  ...
  if(action=="command_status"){
    std::string token=JsonGet(req,"session_token"),id=JsonGet(req,"command_id");Session*s=FindSessionToken(token);if(!s)return "{\"ok\":false,\"error\":\"invalid or expired session token\"}";
...
static void GatewayServer() {
  ...
  while(g_serverRunning){fd_set f;FD_ZERO(&f);FD_SET(listener,&f);timeval tv{1,0};if(select(0,&f,nullptr,nullptr,&tv)<=0)continue;SOCKET c=accept(listener,nullptr,nullptr);if(c==INVALID_SOCKET)continue;
    std::string req;char b[4096];int n;while((n=recv(c,b,sizeof(b),0))>0){req.append(b,n);if(req.find('\n')!=std::string::npos||req.size()>1024*1024)break;}
    std::string res=HandleGateway(req)+"\n";send(c,res.data(),(int)res.size(),0);shutdown(c,SD_BOTH);closesocket(c);
...
static std::string GatewayCall(const std::string& json) {
  ...
  std::string q=json+"\n";send(s,q.data(),(int)q.size(),0);shutdown(s,SD_SEND);std::string r;char b[4096];int n;while((n=recv(s,b,sizeof(b),0))>0)r.append(b,n);closesocket(s);WSACleanup();return r;
}
...
{"name":"session_status","description":"Check a pending session and receive its device-bound token once after local approval.","inputSchema":{"type":"object","properties":{"session_id":{"type":"string"}},"required":["session_id"]}},
...
{"name":"command_status","description":"Check a locally approved/rejected command and retrieve its result.","inputSchema":{"type":"object","properties":{"session_token":{"type":"string"},"command_id":{"type":"string"}},"required":["session_token","command_id"]}}
]})JSON";
}
...
static int McpBridgeMain() {
  wchar_t secret[2048]={};DWORD n=GetEnvironmentVariableW(L"MOMENTUM_BRIDGE_KEY",secret,2048);if(!n)return 3;std::string key=WideToUtf8(secret);
  std::ios::sync_with_stdio(false);std::string line;
  while(std::getline(std::cin,line)){
    std::string method=JsonGet(line,"method"),id;if(!powershellguardian_json::RawField(line,"id",id))id="null";
    if(method=="initialize"){
      std::cout<<"{\"jsonrpc\":\"2.0\",\"id\":"<<id<<",\"result\":{\"protocolVersion\":\"2025-03-26\",\"capabilities\":{\"tools\":{}},\"serverInfo\":{\"name\":\"PowerShell Guardian MCP Bridge\",\"version\":\"1.1.1\"},\"instructions\":\"Begin every new chat with new_session_request. Whitelisted tools run automatically. A session may be locally unlocked; deletion always requires local approval and file creation remains rate-limited. Use execute_command for arbitrary PowerShell.\"}}\n"<<std::flush;
    } else if(method=="tools/list") {
      std::cout<<"{\"jsonrpc\":\"2.0\",\"id\":"<<id<<",\"result\":"<<ToolListJson()<<"}\n"<<std::flush;
    } else if(method=="tools/call") {
      std::string params,arguments,name,token,request;
      powershellguardian_json::RawField(line,"params",params);
      powershellguardian_json::StringField(params,"name",name);
      if(!powershellguardian_json::RawField(params,"arguments",arguments))arguments="{}";
      powershellguardian_json::StringField(arguments,"session_token",token);
...
static LRESULT CALLBACK InputProc(HWND h,UINT m,WPARAM w,LPARAM l){
  ...
  if(m==WM_CREATE){s=(InputState*)((CREATESTRUCTW*)l)->lpCreateParams;SetWindowLongPtrW(h,GWLP_USERDATA,(LONG_PTR)s);
    ...
    for(HWND control:{label,s->edit,ok,cancel})if(control)SendMessageW(control,WM_SETFONT,(WPARAM)font,TRUE);SetFocus(s->edit);return 0;}
...
static bool InputBox(HWND owner,const std::wstring& title,const std::wstring& label,std::wstring& value,bool password=false){
  ...
  MONITORINFO monitor{sizeof(monitor)};if(GetMonitorInfoW(MonitorFromWindow(owner,MONITOR_DEFAULTTONEAREST),&monitor)){int left=(int)monitor.rcWork.left,top=(int)monitor.rcWork.top,right=(int)monitor.rcWork.right,bottom=(int)monitor.rcWork.bottom;x=std::max(left,std::min(x,right-width));y=std::max(top,std::min(y,bottom-height));}
...
static std::wstring NormalizeTunnelId(std::wstring id){
  while(!id.empty()&&iswspace(id.front()))id.erase(id.begin());while(!id.empty()&&iswspace(id.back()))id.pop_back();
  ...
  if(hex32)return L"tunnel_"+compact;return id;
}
...
static bool CheckTunnelEndpoint(const char* path,int attempts){
  ...
  for(int attempt=0;attempt<attempts&&!ok;attempt++){if(!TunnelAlive())break;SOCKET s=socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);sockaddr_in a{};a.sin_family=AF_INET;a.sin_port=htons(17655);inet_pton(AF_INET,"127.0.0.1",&a.sin_addr);
    if(connect(s,(sockaddr*)&a,sizeof(a))!=SOCKET_ERROR){send(s,request.c_str(),(int)request.size(),0);char b[1024]={};int n=recv(s,b,sizeof(b)-1,0);if(n>0&&strstr(b," 200 "))ok=true;}closesocket(s);if(!ok)Sleep(500);
...
static bool ChooseTunnelClient(){
  ...
  OPENFILENAMEW ofn{sizeof(ofn)};ofn.hwndOwner=g_main;ofn.lpstrFilter=filter;ofn.lpstrFile=file;ofn.nMaxFile=32768;ofn.lpstrTitle=L"Select tunnel-client.exe";ofn.Flags=OFN_FILEMUSTEXIST|OFN_PATHMUSTEXIST|OFN_NOCHANGEDIR;
  if(!GetOpenFileNameW(&ofn)||!ExistingFile(file))return false;g_tunnelClientPath=file;SaveTunnelConfig();Audit("SYSTEM","CHANGE TUNNEL CLIENT","LOCAL","SUCCESS");return true;
}
...
static std::vector<EnvironmentBackup> SetTunnelEnvironment(){
  ...
  backups.push_back(SetTemporaryEnvironment(L"MOMENTUM_BRIDGE_KEY",g_bridgeSecret));
...
static ProcessResult RunTunnelTool(const std::wstring& exe,const std::wstring& arguments,const char* stage,DWORD timeoutMs){
  ...
  STARTUPINFOW si{sizeof(si)};PROCESS_INFORMATION pi{};si.dwFlags=STARTF_USESHOWWINDOW|STARTF_USESTDHANDLES;si.wShowWindow=SW_HIDE;si.hStdInput=inputRead;si.hStdOutput=log;si.hStdError=log;
  ...
  if(!GetExitCodeProcess(pi.hProcess,&result.exitCode))result.windowsError=GetLastError();CloseHandle(pi.hProcess);
...
static bool VerifyBridgeProtocol(const std::wstring& bridgeExecutable){
  ...
  STARTUPINFOW si{sizeof(si)};PROCESS_INFORMATION pi{};si.dwFlags=STARTF_USESHOWWINDOW|STARTF_USESTDHANDLES;si.wShowWindow=SW_HIDE;si.hStdInput=stdinRead;si.hStdOutput=stdoutWrite;si.hStdError=stdoutWrite;
  ...
  CloseHandle(pi.hThread);const std::string request="{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{\"protocolVersion\":\"2025-03-26\",\"capabilities\":{},\"clientInfo\":{\"name\":\"PowerShellGuardianPreflight\",\"version\":\"1.1.1\"}}}\n";DWORD written=0;BOOL sent=WriteFile(stdinWrite,request.data(),(DWORD)request.size(),&written,nullptr);CloseHandle(stdinWrite);
  ...
  bool valid=sent&&written==request.size()&&!timedOut&&exitCode==0&&output.find("PowerShell Guardian MCP Bridge")!=std::string::npos&&output.find("\"version\":\"1.1.1\"")!=std::string::npos;
...
static bool PrepareAndVerifyTunnelProfile(const std::wstring& exe,const std::wstring& mcpCommand,std::wstring& profile){
  ...
  if(initialized.exitCode!=0){RestoreEnvironment(environment);AppendFileUtf8(TunnelLogPath(),"ERROR: PROFILE INIT could not replace the deterministic PowerShellGuardian profile.\r\n");Audit("SYSTEM","TUNNEL PROFILE INIT","LOCAL","FAILED "+std::to_string(initialized.exitCode));return false;}
...
static bool StartTunnel() {
  if(TunnelAlive())return true;g_tunnelReady=false;if(g_apiKey.empty()||!ValidTunnelId(g_tunnelId))return false;
  ...
  if(!ExistingFile(bridgeExecutable)){WriteFileUtf8(TunnelLogPath(),"ERROR: PowerShellGuardianBridge.exe is missing. Run PowerShellGuardianSetup.exe 1.1.1 or newer.\r\n");return false;}
...
static void AddCell(HWND h,int row,int col,const std::wstring&text){if(col==0){LVITEMW i{};i.mask=LVIF_TEXT;i.iItem=row;i.pszText=(LPWSTR)text.c_str();ListView_InsertItem(h,&i);}else ListView_SetItemText(h,row,col,(LPWSTR)text.c_str());}

static void RefreshUi(){
  if(!g_main)return;bool alive=TunnelAlive();if(alive&&!g_tunnelReady){bool ready=CheckTunnelEndpoint("/readyz",1);bool healthy=ready&&CheckTunnelEndpoint("/healthz",1);if(ready&&healthy){g_tunnelReady=true;Audit("SYSTEM","VERIFY TUNNEL","LOCAL","READY");}}
...
static void ApproveNewestSession(){
  int idx=-1;std::string code,id;{std::lock_guard<std::mutex> lock(g_mutex);for(int i=(int)g_sessionData.size()-1;i>=0;i--)if(!g_sessionData[i].approved&&!g_sessionData[i].denied){idx=i;code=g_sessionData[i].code;id=g_sessionData[i].id;break;}}
  if(idx<0)return;std::wstring msg=L"Nova ChatGPT seja zahteva dovoljenje\n\nVerification Code: "+Utf8ToWide(code)+L"\n\nAPPROVE SESSION?";
  int answer=MessageBoxW(g_main,msg.c_str(),APP_NAME,MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2|MB_TOPMOST);
  {std::lock_guard<std::mutex> lock(g_mutex);if(idx>=(int)g_sessionData.size()||g_sessionData[idx].id!=id)return;auto&s=g_sessionData[idx];
    if(answer==IDYES){s.approved=true;s.token=RandomToken();s.deviceHash=DeviceHash();s.tokenHash=Sha256(s.token+"|"+s.deviceHash);s.lastActive=Now();s.expires=s.lastActive+g_timeoutMinutes*60;Audit(s.id,"SESSION","APPROVED","token_ref "+s.tokenHash.substr(0,16));SaveSessions();}
    else{s.denied=true;Audit(s.id,"SESSION","DENIED","no access granted");}}
  RefreshUi();
}
...
static void DecideCommand(bool approve){
  ...
  std::string id;{std::lock_guard<std::mutex> lock(g_mutex);if(row>=(int)g_commandData.size())return;auto&c=g_commandData[row];if(c.status!="WAITING APPROVAL")return;id=c.id;
    if(!approve){c.status="REJECTED";c.result="Rejected locally";Audit(c.sessionId,c.name,"REJECTED",c.result);}}
  if(!approve){RefreshUi();return;}
  std::wstring details,caption=L"PowerShell Guardian - Local Approval";bool deletion=false;{std::lock_guard<std::mutex> lock(g_mutex);auto&c=g_commandData[row];deletion=c.risk=="DELETE";if(deletion){caption=L"PowerShell Guardian - File Deletion Approval";details=L"FILE DELETION REQUIRES YOUR LOCAL APPROVAL\n\nCOMMAND: "+Utf8ToWide(c.name)+L"\nSOURCE: ChatGPT\n\nTARGET / COMMAND:\n"+Utf8ToWide(c.payload)+L"\n\nAllow this file deletion?";}else details=L"COMMAND: "+Utf8ToWide(c.name)+L"\n\nSOURCE: ChatGPT\nRISK: "+Utf8ToWide(c.risk)+L"\n\n"+Utf8ToWide(c.payload)+L"\n\nExecute this command?";}
  ...
  {std::lock_guard<std::mutex> lock(g_mutex);for(auto&c:g_commandData)if(c.id==id)c.status="RUNNING";}RefreshUi();
  std::thread([id]{PendingCommand copy;{std::lock_guard<std::mutex> lock(g_mutex);for(auto&c:g_commandData)if(c.id==id)copy=c;}
    std::string result=ExecuteApproved(copy);{std::lock_guard<std::mutex> lock(g_mutex);for(auto&c:g_commandData)if(c.id==id){c.result=result;c.status=result.rfind("ERROR",0)==0||result.rfind("BLOCKED",0)==0?"FAILED":"SUCCESS";Audit(c.sessionId,c.name,"APPROVED",c.status);}}
...
static void PromptNewestCommand(){
  ...
  if(IsIconic(g_main))ShowWindow(g_main,SW_RESTORE);BringWindowToTop(g_main);SetForegroundWindow(g_main);
  DecideCommand(true);
}

static void DeleteSelectedSession(){int row=Selected(g_sessions);if(row<0)return;std::lock_guard<std::mutex> lock(g_mutex);if(row<(int)g_sessionData.size()){Audit(g_sessionData[row].id,"SESSION DELETE","LOCAL","REVOKED");g_sessionData.erase(g_sessionData.begin()+row);SaveSessions();}}
static void DeleteAllSessions(){std::lock_guard<std::mutex> lock(g_mutex);for(auto&s:g_sessionData)Audit(s.id,"DELETE ALL SESSIONS","LOCAL","REVOKED");g_sessionData.clear();SaveSessions();}
...
static void ToggleSelectedSessionAccess(){
  ...
  {std::lock_guard<std::mutex> lock(g_mutex);for(auto& s:g_sessionData)if(s.id==id){s.unattended=unlocking;Audit(s.id,"SESSION ACCESS", "LOCAL",unlocking?"UNLOCKED - deletion still requires approval":"LOCKED - approval restored");SaveSessions();break;}}
  RefreshUi();
}
...
static void ConfigureSecurity(HWND owner){
  int timeout=g_timeoutMinutes,limit=g_fileLimit,window=g_fileWindowMinutes,streak=g_suspiciousFullWindows;
  ...
  g_timeoutMinutes=timeout;g_fileLimit=limit;g_fileWindowMinutes=window;g_suspiciousFullWindows=streak;g_suspiciousGuardEnabled=guard==IDYES;
  if(!SaveSecuritySettings()){MessageBoxW(owner,L"Security settings could not be saved.",APP_NAME,MB_OK|MB_ICONERROR);return;}
  Audit("SYSTEM","SECURITY SETTINGS","LOCAL","timeout "+std::to_string(timeout)+", file limit "+std::to_string(limit)+" per "+std::to_string(window)+" minutes, suspicious windows "+std::to_string(streak)+", guard "+(g_suspiciousGuardEnabled?"enabled":"disabled"));
}
...
static LRESULT CALLBACK MainProc(HWND h,UINT m,WPARAM w,LPARAM l){
  switch(m){
  case WM_CREATE:{HFONT font=(HFONT)GetStockObject(DEFAULT_GUI_FONT);CreateWindowW(L"STATIC",L"PowerShell Guardian Control Center — Version 1.1.1",WS_CHILD|WS_VISIBLE,20,16,450,28,h,nullptr,g_instance,nullptr);
    ...
    Button(h,L"APPROVE",ID_APPROVE,595,440,130);Button(h,L"REJECT",ID_REJECT,735,440,130);
  ...
  case WM_COMMAND:switch(LOWORD(w)){
    ...
    case ID_APPROVE:DecideCommand(true);break;case ID_REJECT:DecideCommand(false);break;case ID_LOGS:{std::wstring logs=g_base+L"\\logs";ShellExecuteW(h,L"open",logs.c_str(),nullptr,nullptr,SW_SHOW);}break;
    case ID_SECURITY:ConfigureSecurity(h);break;
    case ID_EMERGENCY:EmergencyLock();break;}return 0;
  case WM_CLOSE:StopTunnel("APPLICATION CLOSE");DestroyWindow(h);return 0;case WM_DESTROY:g_serverRunning=false;PostQuitMessage(0);return 0;}return DefWindowProcW(h,m,w,l);
}

static void CleanupLoop(){while(g_serverRunning){Sleep(30000);std::lock_guard<std::mutex>lock(g_mutex);long long n=Now();auto old=g_sessionData.size();g_sessionData.erase(std::remove_if(g_sessionData.begin(),g_sessionData.end(),[n](const Session&s){return (s.denied&&!s.suspiciousTerminated)||s.expires<=n;}),g_sessionData.end());if(old!=g_sessionData.size())SaveSessions();if(g_main)PostMessageW(g_main,WM_MS_REFRESH,0,0);}}

static void InitPaths(){wchar_t p[MAX_PATH]={};SHGetFolderPathW(nullptr,CSIDL_COMMON_APPDATA,nullptr,SHGFP_TYPE_CURRENT,p);g_base=std::wstring(p)+L"\\PowerShellGuardian";std::vector<wchar_t> executable(32768);DWORD executableLength=GetModuleFileNameW(nullptr,executable.data(),(DWORD)executable.size());if(executableLength&&executableLength<executable.size())g_executablePath.assign(executable.data(),executableLength);g_install=g_executablePath;size_t z=g_install.find_last_of(L"\\/");if(z!=std::wstring::npos)g_install.resize(z);EnsureDirs();LoadWhitelist();g_bridgeSecret=LoadSecret(L"bridge.key");if(g_bridgeSecret.empty()){g_bridgeSecret=Utf8ToWide(RandomToken(48));if(!SaveSecret(L"bridge.key",g_bridgeSecret))AppendFileUtf8(g_base+L"\\logs\\tunnel-client.log","ERROR: protected MCP bridge key could not be persisted; it was regenerated for this process.\r\n");}g_apiKey=LoadSecret(L"api.key");std::string tj=ReadFileUtf8(g_base+L"\\config\\tunnel.json");g_tunnelId=NormalizeTunnelId(LoadSecret(L"tunnel.id"));if(!ValidTunnelId(g_tunnelId)){g_tunnelId=NormalizeTunnelId(Utf8ToWide(JsonGet(tj,"tunnel_id")));if(ValidTunnelId(g_tunnelId))SaveSecret(L"tunnel.id",g_tunnelId);}g_tunnelClientPath=Utf8ToWide(JsonGet(tj,"tunnel_client_path"));
  ...
  readBounded("session_timeout_minutes",30,1440,g_timeoutMinutes);readBounded("unattended_file_limit",1,100,g_fileLimit);readBounded("unattended_file_window_minutes",1,1440,g_fileWindowMinutes);readBounded("suspicious_full_windows",2,20,g_suspiciousFullWindows);g_suspiciousGuardEnabled=JsonBool(sj,"suspicious_guard_enabled",true);LoadSessions();}
...
static DWORD WINAPI ServiceHandler(DWORD control,DWORD,LPVOID,LPVOID){if(control==SERVICE_CONTROL_STOP||control==SERVICE_CONTROL_SHUTDOWN){SetServiceState(SERVICE_STOP_PENDING);g_serverRunning=false;Audit("SYSTEM","SERVICE STOP","LOCAL","SUCCESS");}return NO_ERROR;}
...
int WINAPI wWinMain(HINSTANCE inst,HINSTANCE,LPWSTR cmd,int show){
  ...
  std::thread server(GatewayServer);while(!g_serverRunning)Sleep(20);std::thread cleaner(CleanupLoop);RefreshUi();if(g_apiKey.size()<8||!ValidTunnelId(g_tunnelId))PostMessageW(g_main,WM_MS_SETUP,0,0);
